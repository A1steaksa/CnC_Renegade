...
public:
	...
	DialogEvent( EventID event, DialogBaseClass* dialog) : TypedEventPtr<DialogEvent, DialogBaseClass>(dialog), mEvent(event){
	}
...
public:
	...
	virtual PopupDialogClass* As_PopupDialogClass(void){
		return NULL;
	}

	virtual ChildDialogClass* As_ChildDialogClass(void){
		return NULL;
	}


	int Get_Dlg_ID(void) const {
		return DialogResID;
	}
	...
	//	Position control
	//
	const RectClass& Get_Rect(void) const {
		return Rect;
	}
	...
	int Get_Control_Count(void) const {
		return ControlList.Count();
	}

	DialogControlClass*	Get_Control( int index ) const {
	...
	//	Title access
	//
	void Get_Title( WideStringClass* title ){
		*title = Title;
	}

	void Set_Title( const WCHAR* title ){
		Title = title;
	}
	...
	virtual bool Wants_Activation(void){
		return true;
	}
	...
	//	Transition control
	//
	virtual DialogTransitionClass* Get_Transition_In( DialogBaseClass* prev_dlg ){
		return NULL;
	}

	virtual DialogTransitionClass* Get_Transition_Out( DialogBaseClass* next_dlg ){
		return NULL;
	}
	
	virtual void Set_Controls_Hidden(bool onoff){
		AreControlsHidden = onoff;
	}

	virtual bool Are_Controls_Hidden(void) const {
	...
	static DEFAULT_DLG_CMD_HANDLER Get_Default_Command_Handler(void){
