...
// Macros.
// NOTE 0: Word wrap logic does not apply to Han characters (Chinese, Japanese &
// Korean).
//			  Therefore treat each of these characters as a word
//which can be preceeded by a line break.
// NOTE 1: This is a simplification. Some Korean characters should not be line
// break characters.
...
Render2DSentenceClass::Render2DSentenceClass( void )
    : Font( NULL ), Location( 0.0F, 0.0F ), Cursor( 0.0F, 0.0F ), TextureOffset( 0, 0 ),
    TextureStartX( 0 ), CurSurface( NULL ), CurrTextureSize( 0 ), MonoSpaced( false ),
    IsClippedEnabled( false ), ClipRect( 0, 0, 0, 0 ), BaseLocation( 0, 0 ),
    LockedPtr( NULL ), LockedStride( 0 ), TextureSizeHint( 0 ), WrapWidth( 0 ),
    TabStop( 5.0 ), DrawExtents( 0, 0, 0, 0 ),
    Renderers( sizeof( PreAllocatedRenderers ) / sizeof( RendererDataStruct ),
...
const WCHAR* Render2DSentenceClass::Find_Row_Start( const WCHAR* text,
    int row_index ) {
    if( row_index == 0 ) {
        return text;
    }
...
void Render2DSentenceClass::Build_Textures( void ) {
    ...
    for( int index = 0; index < PendingSurfaces.Count(); index++ ) {
        ...
        TextureClass* new_texture =
            new TextureClass( desc.Width, desc.Width, WW3D_FORMAT_A4R4G4B4,
        ...
        for( int renderer_index = 0;
            renderer_index < surface_info.Renderers.Count();
            renderer_index++ ) {
    ...
    for( int index = 0; index < SentenceData.Count(); index++ ) {
        ...
        if( data.Surface != curr_surface ) {
            ...
            if( found == false ) {
                ...
                for( int surface_index = 0;
                    surface_index < PendingSurfaces.Count(); surface_index++ ) {
                    PendingSurfaceStruct& surface_info =
                        PendingSurfaces[surface_index];
        ...
        if( IsClippedEnabled ) {
            ...
            if( screen_rect.Right <= ClipRect.Left ||
                ...
                float percent = ( ( clipped_rect.Left - screen_rect.Left ) /
                    screen_rect.Width() );
                clipped_uv_rect.Left =
                    uv_rect.Left + ( uv_rect.Width() * percent );

                percent = ( ( clipped_rect.Right - screen_rect.Left ) /
                    screen_rect.Width() );
                clipped_uv_rect.Right =
                    uv_rect.Left + ( uv_rect.Width() * percent );

                percent = ( ( clipped_rect.Top - screen_rect.Top ) /
                    screen_rect.Height() );
                clipped_uv_rect.Top =
                    uv_rect.Top + ( uv_rect.Height() * percent );

                percent = ( ( clipped_rect.Bottom - screen_rect.Top ) /
                    screen_rect.Height() );
                clipped_uv_rect.Bottom =
                    uv_rect.Top + ( uv_rect.Height() * percent );
...
void Render2DSentenceClass::Build_Sentence( const WCHAR* text ) {
    ...
    while( text != NULL ) {
        ...
        bool encountered_break_char =
            ( IS_BREAK_CHAR( ch ) || ch == L'\n' || ch == 0 || ch == L'\t' );
...
FontCharsClass::FontCharsClass( void )
    : OldGDIFont( NULL ), OldGDIBitmap( NULL ), GDIFont( NULL ), GDIBitmap( NULL ),
    GDIBitmapBits( NULL ), MemDC( NULL ), CurrPixelOffset( 0 ), PointSize( 0 ),
    CharHeight( 0 ), UnicodeCharArray( NULL ), FirstUnicodeChar( 0xFFFF ),
    LastUnicodeChar( 0 ), IsBold( false ),
    BufferList( sizeof( PreAllocatedBufferList ) / sizeof( uint16* ),
        PreAllocatedBufferList ) {
...
const FontCharsClass::CharDataStruct* FontCharsClass::Store_GDI_Char( WCHAR ch ) {
    ...
    //
    if( ( ch == 'W' || ch == 'V' ) &&
        ( GDIFontName.Compare_No_Case( "Arial MT" ) == 0 ) ) {
        x_pos = 1;
        char_size.cx += 1;
    }
...
void FontCharsClass::Create_GDI_Font( const char* font_name ) {
    ...
    //
    int font_height =
        -MulDiv( PointSize, ::GetDeviceCaps( screen_dc, LOGPIXELSY ), 72 );
    ...
    GDIFont = ::CreateFont( font_height, 0, 0, 0, bold, italic, FALSE, FALSE,
...
void FontCharsClass::Grow_Unicode_Array( WCHAR ch ) {
    ...
    if( UnicodeCharArray != NULL ) {
        ...
        ::memcpy( &new_array[start_offset], UnicodeCharArray,
            sizeof( CharDataStruct* ) * old_count );
