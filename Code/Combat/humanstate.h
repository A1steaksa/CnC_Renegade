...
public:
	...
	void Set_State_Timer( float timer ){
		StateTimer = timer;
	}

	void Toggle_State_Flag( int flag ){
		StateFlags ^= flag;
	}

	bool Get_State_Flag( int flag ){
		return ( StateFlags & flag ) != 0;
	}

	void Drop_Weapon(void){
		WeaponHoldTimer = 0.001f;
	}

	void Raise_Weapon(void){
		WeaponHoldTimer = 10;
	}
	...
	void Set_Turn_Velocity( float vel ){
		TurnVelocity = vel;
	}
