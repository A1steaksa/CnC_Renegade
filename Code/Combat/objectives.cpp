...
const Vector3& Objective::Type_To_Color(void){
	...
	// Dim the colors ifthis objective has been accomplished
...
void ObjectiveManager::Set_Objective_Status( int id, int status ){
	...
	if( objective != NULL ){
		...
		// Special case changing an objective from hidden to pending... (Note:  for// a completed objective, we display the normal message even ifit was hidden).
		WideStringClass message;
