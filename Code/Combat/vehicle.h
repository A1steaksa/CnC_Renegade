...
// Script enabling of transitions
    ...
    ScriptableGameObj* Get_Lock_Owner( void ) {
        return LockOwner.Get_Ptr();
    }

    static void	Set_Default_Driver_Is_Gunner( bool flag ) {
        DefaultDriverIsGunner = flag;    
}
    bool	Get_Driver_Is_Gunner( void ) {
        return DriverIsGunner;
    }
    void	Toggle_Driver_Is_Gunner( void ) {
