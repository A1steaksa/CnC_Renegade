...
public:
	...
	DialogueClass* Get_Dialog_List(void){
		return DialogList;
	}
...
public:
	...
	void Interpret_Sc_State_Data( HumanStateClass::HumanStateType state, int sub_state, LPCSTR trans_name, const Vector3& velocity, const Vector3& sc_position );
	...
	bool Is_Upright(void){
		return Get_State() == HumanStateClass::UPRIGHT;
	}

	bool Is_Wounded(void){
	...
	bool Is_On_Ladder(void){
		return Get_State() == HumanStateClass::LADDER;
	}

	bool Is_State_Locked(void){
		return HumanState.Is_Locked();
	}

	bool Is_In_Vehicle(void){
		return Get_State() == HumanStateClass::IN_VEHICLE;
	}

	void Reset_Loiter_Delay(void){
		HumanState.Reset_Loiter_Delay();
	}

	void Set_Loiters_Allowed( bool allowed ){
		HumanState.Set_Loiters_Allowed( allowed );
	}
	...
	VehicleGameObj* Get_Vehicle(void){
		return Vehicle;
	}
	...
	StringClass Get_Animation_Name(void){
	...
	HumanStateClass* Get_Human_State(void){
		return &HumanState;
	}
	...
	void Cancel_Look_At(void){
		HeadLookDuration = 0.001F;
	}

	void Look_Random( float time );
	
	bool Is_Looking(void){
	...
	// Speech support
	//
	DynamicSpeechAnimClass* Get_Facial_Anim(void){
		return SpeechAnim;
	}
	...
	SoldierAIState Get_AI_State(void) const {
		return AIState;
	}
	void Set_AI_State( SoldierAIState state );

	// Innate observer access
	void Set_Innate_Observer( SoldierObserverClass* observer ){
		InnateObserver = observer;
	}
	SoldierObserverClass* Get_Innate_Observer(void){
		return InnateObserver;
	}
	void Clear_Innate_Observer(void){
		InnateObserver = NULL;
	}
	...
	void Give_Key( int key_number ){
		KeyRing |= ( 1 << key_number );
	}
	void Remove_Key( int key_number ){
		KeyRing &= ~( 1 << key_number );
	}
	bool Has_Key( int key_number ){
	...
	virtual bool		Allow_Special_Damage_State_Lock(void){
		return ( Is_Human_Controlled() == false );
	}
