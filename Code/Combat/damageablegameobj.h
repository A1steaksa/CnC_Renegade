...
public:
	...
	int Get_Name_ID(void) const {
		return TranslatedNameID;
	}

	// Encyclopedia information
	EncyclopediaMgrClass::TYPE Get_Encyclopedia_Type(void) const {
		return EncyclopediaType;
	}

	int Get_Encyclopedia_ID(void) const {
		return EncyclopediaID;
	}

	// Icon information
	const StringClass& Get_Icon_Filename(void) const {
	...
	const DefenseObjectDefClass& Get_DefenseObjectDef(void) const {
		return DefenseObjectDef;
	}

	// Accessors (added as needed)
	int Get_Default_Player_Type(void) const {
...
public:
	...
	virtual bool Is_Targetable(void) const {
		return !Get_Definition().NotTargetable;
	}
