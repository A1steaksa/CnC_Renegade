...
public:
	...
	// Font control
	//
	void Set_Heading_Font( const char* font_name ){
		HeadingFontName = font_name;
	}
	...
	bool Are_Columns_Displayed(void) const {
		return AreColumnsDisplayed;
	}
	
	void Display_Columns( bool onoff ){
		AreColumnsDisplayed = onoff; IsViewDirty = true; IsWindowDirty = true;
	}
...
public:
	...
	// Name access
	//
	const WCHAR* Get_Name(void) const {
		return Name;
	}
	...
	void Set_User_Data( uint32 user_data ){
		UserData = user_data;
	}
...
public:
	...
	void Set_Item_Text( int index, const WCHAR* text ){
		Items[index]->Set_Name( text );
	}
	const WCHAR* Get_Item_Text( int index ) const {
		return Items[index]->Get_Name();
	}

	void Set_Item_Color( int index, const Vector3& color ){
		Items[index]->Set_Color( color );
	}
	const Vector3& Get_Item_Color( int index ) const {
		return Items[index]->Get_Color();
	}

	void Set_Item_Data( int index, uint32 data ){
		Items[index]->Set_User_Data( data );
	}
	uint32 Get_Item_Data( int index ) const {
		return Items[index]->Get_User_Data();
	}
