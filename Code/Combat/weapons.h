...
public:
	...
	const WeaponDefinitionClass* Get_Definition(void){
		return Definition;
	}
	...
	const char* Get_HUD_Icon_Texture_Name(void){
	...
	const char* Get_Back_Model_Name(void){
	...
	float Get_Key_Number(void){
		return Definition->KeyNumber;
	}

	bool Get_Can_Snipe(void){
		return Definition->CanSnipe;
	}

	float Get_Rating(void){
		return Definition->Rating;
	}

	float Get_Effective_Range(void){
		return PrimaryAmmoDefinition->EffectiveRange;
	}

	float Get_Primary_Fire_Rate(void){
		return PrimaryAmmoDefinition->RateOfFire;
	}

	const char* Get_First_Person_Model_Name(void) const {
	...
	float Get_Recoil_Time(void){
		return Definition->RecoilTime;
	}

	float Get_Recoil_Scale(void){
	...
	bool Is_Reload_Needed(void){
		return !Is_Loaded() && Is_Reload_OK();
	}

	float Get_Range(void);

	bool Is_Model_Update_Needed(void){
	...
	bool Is_Anim_Update_Needed(void){
		return ( NextAnimState != CurrentAnimState );
	}
	
	void Reset_Anim_Update(void){
		CurrentAnimState = NextAnimState;
	}
	
	WeaponAnimState	Get_Anim_State(void){
		return CurrentAnimState;
	}

	int Get_Total_Rounds_Fired(void){
	...
	*/
	...
	WeaponStateType	Get_State(void){
		return State;
	}
	...
	// Trigger State
	void Set_Safety( bool saftey ){
		SafetySet = saftey;
	}

	bool Is_Safety_Set(void){
		return SafetySet;
	}
	...
	void Init_Muzzle_Flash( RenderObjClass* robj ){
		MuzzleFlash.Init( robj );
	}

	void Update_Muzzle_Flash( bool flashA0, bool flashA1 ){
		MuzzleFlash.Update( flashA0, flashA1 );
	}

	bool Does_Weapon_Exist(void){
		return WeaponExists;
	}
	
	void Set_Weapon_Exists( bool exists ){
		WeaponExists = exists;
	}
