...
return TimeLimitMinutes > 0;
	...
	//	Version information
	//
	bool	Do_Exe_Versions_Match( void ) {
		return DoExeVersionsMatch;
	}
	bool	Do_String_Versions_Match( void ) {
		return DoStringVersionsMatch;
	}

	//
	//	Map cycle looping support
	//
	bool	Is_Map_Cycle_Over( void ) {
		return IsMapCycleOver;
	}
	void	Set_Is_Map_Cycle_Over( bool onoff ) {
		IsMapCycleOver = onoff;
	}
	bool	Do_Maps_Loop( void ) {
		return DoMapsLoop;
	}
	void	Set_Do_Maps_Loop( bool onoff ) {
		DoMapsLoop = onoff;
	}
	...
	//
	virtual bool	Is_Single_Player( void )			const {
		return false;
	}
	virtual bool	Is_Skirmish( void )					const {
		return false;
	}
	virtual bool	Is_Cnc( void )						const {
		return false;	
}

	virtual cGameDataSinglePlayer* As_Single_Player( void ) {
		return NULL;
	}
	virtual cGameDataSkirmish* As_Skirmish( void ) {
		return NULL;
	}
	virtual cGameDataCnc* As_Cnc( void ) {
		return NULL;
	}

	virtual int		Get_Min_Players( void ) const {		
return 2;
	}
	...
	float				Get_Intermission_Time_Remaining( void ) {
		return IntermissionTimeRemaining;
	}
	void				Set_Intermission_Time_Remaining( float time ) {
		IntermissionTimeRemaining = time;
	}
	float				Get_Maximum_World_Distance( void ) {		
return MaximumWorldDistance;	
}
	void				Set_Maximum_World_Distance( float distance );
	unsigned long	Get_Frame_Count( void ) const {
		return FrameCount;
	}
	LPSYSTEMTIME	Get_Game_Start_Time( void ) {
		return &GameStartTime;
	}
	int				Get_Duration_Seconds( void );
	void				Set_Min_Qualifying_Time_Minutes( int minutes );
	int				Get_Min_Qualifying_Time_Minutes( void ) {
		return MinQualifyingTimeMinutes;	
}
	...
	void				Set_Ini_Filename( const StringClass& name ) {
		IniFilename = name;
	}
	const StringClass& Get_Ini_Filename() {
		return IniFilename;	
}
	virtual bool	Is_Gameplay_Permitted( void );

	virtual	bool	Remember_Inventory( void )	const {
		return false;
	}

	void				Set_Winner_ID( int id ) {
		mWinnerID = id;
	}
	int				Get_Winner_ID( void ) const {
		return mWinnerID;	
}

	static void		Set_Hosted_Game_Number( int number ) {
		HostedGameNumber = number;
	}
	static void		Increment_Hosted_Game_Number( void ) {
		HostedGameNumber++;
	}
	static int		Get_Hosted_Game_Number( void ) {
	...
	*/
	...
	static	bool	Is_Manual_Restart( void ) {
		return IsManualRestart;
	}
	static	void	Set_Manual_Restart( bool is_manual_restart ) {		
IsManualRestart = is_manual_restart;
	}

	static	bool	Is_Manual_Exit( void ) {
		return IsManualExit;
	}
	static	void	Set_Manual_Exit( bool is_manual_exit ) {		
IsManualExit = is_manual_exit;	
}


	static	void					Set_Win_Text( WideStringClass& text );
	static	WideStringClass	Get_Win_Text( void ) {
		return WinText;	
}
	...
	const WideStringClass& Get_Mvp_Name( void ) const {
		return MvpName;	
}

	void				Set_Mvp_Count( int count );
	void				Increment_Mvp_Count( void ) {
		MvpCount++;
	}
	const int		Get_Mvp_Count( void ) const {
		return MvpCount;
	}

	void				Set_Win_Type( WinTypeEnum type );
	WinTypeEnum		Get_Win_Type( void ) const {
		return WinType;	
}

	void				Set_Game_Duration_S( DWORD seconds );
	DWORD				Get_Game_Duration_S( void ) const {
	...
	void					Set_Settings_Description( WideStringClass desc ) {
